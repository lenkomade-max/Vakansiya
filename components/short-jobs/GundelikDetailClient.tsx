...
type GundelikDetailClientProps = {
    initialGundelik: Job
    jobId: string
}

export default function GundelikDetailClient({ initialGundelik, jobId }: GundelikDetailClientProps) {
    const router = useRouter()
    const [showPhone, setShowPhone] = useState(false)
    const [showComplaintModal, setShowComplaintModal] = useState(false)

    // Загружаем только auth с кешированием
    ...
    const categoryImage = job.category_name
        ? getCategoryImage({ name: job.category_name, image_url: (job as any).category_info?.image_url })
        : null

    const categoryAlt = job.category_name
        ? getCategoryImageAlt({ name: job.category_name })
        : 'Gündəlik iş'
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div className="relative aspect-video bg-gray-100 overflow-hidden">
                                {categoryImage ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center">
                                        <CategoryIcon category={job.category as ShortJobCategory} size="xl" showBackground={false} />
                                    </div>
                                )}
                                ...
                                {/* Salary badge (always shown for gundelik) */}
                                {job.salary && (
                    ...
                    <div className="lg:col-span-1">
                                ...
                                {job.parent_category_name && (
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm font-semibold text-black">{job.parent_category_name}</p>
                                        </div>
                                    </div>
                                )}

                                {job.category_name && job.category_name !== job.parent_category_name && (
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center">
                                            <svg className="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm font-semibold text-black">{job.category_name}</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6 sticky top-6">
                            ...
                            {/* Recruiter contact (if employer_phone exists) */}
                            {job.employer_phone && (
                                <div className="mb-6 pb-4 border-b border-gray-200">
                                    <p className="text-sm text-gray-600 mb-2">
                                        Rekruter / HR / Agentlik:
                                    </p>
                                    {job.contact_name && (
                                    ...
                                    {!showPhone ? (
                                        <button
                                            ...
                                            className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gray-100 text-gray-700 rounded-lg font-semibold text-sm hover:bg-gray-200 transition-all"
                                        >
                                        ...
                                        <div className="space-y-2">
                                            <a
                                                ...
                                                className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gray-800 text-white rounded-lg font-semibold text-sm hover:bg-gray-900 transition-all"
                                            >
                                                <PhoneIcon className="w-4 h-4" />
                                                {job.contact_phone}
                                            </a>
                                            <a
                                                ...
                                                className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-green-600 text-white rounded-lg font-semibold text-sm hover:bg-green-700 transition-all"
                                            >
                            ...
                            {/* Employer contact (main) */}
                            <div>
                                <p className="text-sm text-gray-600 mb-2">
                                    {job.employer_phone ? 'İşəgötürən:' : (job.contact_name ? 'Əlaqə şəxsi:' : 'Əlaqə:')}
                                </p>
                                {!job.employer_phone && job.contact_name && (
                                    <p className="text-base font-semibold text-black mb-3">
                                        {job.contact_name}
                                    </p>
                                )}
                                ...
                                {!showPhone ? (
                                    ...
                                    >
                                        ...
                                        {job.employer_phone ? 'Nömrəni göstər' : 'Əlaqə'}
                                    </button>
                                ) : (
                                    <div className="space-y-3">
                                        <a
                                            href={`tel:${job.employer_phone || job.contact_phone}`}
                                            className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-black text-white rounded-xl font-bold text-base hover:bg-gray-800 transition-all active:scale-95"
                                        >
                                            <PhoneIcon className="w-5 h-5" />
                                            {job.employer_phone || job.contact_phone}
                                        </a>
                                        <a
                                            href={`https://wa.me/${(job.employer_phone || job.contact_phone).replace(/\D/g, '')}`}
                                            target="_blank"
                                            rel="noopener noreferrer"
